...
dal.getContractorWeeklyRemittanceData=function(param,cb){
   ...
   var weekEnding = new Date();
    ...
    sql += "Select pay.subcontractor_id as subId, pay.week_ending as week_ending, pay.payment_date as payment_date, pay.month_ending_date as month_ending_date, pay.deduction_rate, pay.vat_rate, "
    ...
    sql += "(pay.gross * (pay.deduction_rate / 100)) as tax, ((pay.gross + pay.materials) * (pay.vat_rate / 100)) as vat, ((pay.gross + pay.materials) - (pay.gross * (pay.deduction_rate / 100))) as net, ";
    ...
    sql += "((((((pay.gross + pay.materials) - (pay.gross * (pay.deduction_rate / 100))) + ((pay.gross + materials) * (pay.vat_rate / 100))) - COALESCE((select(select SUM(amount) from tblPayrollDeductions where payroll_id = pay.id)),0)) - pay.fee)) as payable "
    ...
    sql += "select ";
    sql += "CONCAT('£', FORMAT(SUM(pay.gross + pay.materials), 2)) as grossSum, CONCAT('£', FORMAT(SUM(pay.materials), 2)) as materialsSum, "
    sql += "CONCAT('£', FORMAT(COALESCE((select SUM(amount) from tblPayrollDeductions where payroll_id in (select id from Payroll where DATE_FORMAT(payment_date, '%d-%m-%Y')"
    sql += util.format("= '%s' and pay.contractor_id = %d", param.paymentDate, param.id);
    sql += ")),0), 2)) as deductionsSum, CONCAT('£', FORMAT(SUM(pay.gross), 2)) as taxableSum, CONCAT('£', FORMAT(SUM((pay.gross * (pay.deduction_rate / 100))), 2)) as taxSum, ";
    sql += "CONCAT('£', FORMAT(SUM(((pay.gross + pay.materials) - (pay.gross * (pay.deduction_rate / 100)))), 2)) as netSum, CONCAT('£', FORMAT(SUM(pay.fee), 2)) as feeSum,"
    sql += "CONCAT('£', FORMAT(SUM(((((((pay.gross + pay.materials) - (pay.gross * (pay.deduction_rate / 100))) + ((pay.gross + pay.materials) * (pay.vat_rate / 100))) - COALESCE((select(select SUM(amount) from tblPayrollDeductions where payroll_id = pay.id)),0)) - pay.fee))), 2)) as payableSum, "
    sql += "CONCAT('£', FORMAT(con.tlcins * COUNT(pay.id), 2)) as tlcSum, "
    sql += "CONCAT('£', FORMAT(IF(con.payer_type = 'Gross Payer', SUM((pay.gross)* pay.deduction_rate/100), 0), 2)) as taxHeldSum, "
    sql += "CONCAT('£', FORMAT(SUM((pay.gross - (pay.gross *(pay.vat_rate / 100)))) + SUM(pay.vat_rate) + SUM((pay.gross *(pay.vat_rate / 100))) + SUM((con.tlcins * (select COUNT(*) from Payroll where DATE_FORMAT(payment_date, '%d-%m-%Y')";
    sql += util.format("= '%s' and pay.contractor_id = %d", param.paymentDate, param.id);
...
dal.getBatchData=function(param,cb){
    ...
    var sql = "select id, fileName, dated, if(isComplete = 1, 'Complete', 'In Progress') as status, userName from Batch order by id desc";
...
dal.subContractorMonthlyStatement=function(param,cb){
    ...
    sql += util.format("SELECT CONCAT('£', FORMAT(SUM(gross), 2)) as gross, CONCAT('£', FORMAT(SUM(materials), 2)) as materialCost, CONCAT('£', FORMAT(SUM((gross - materials)*deduction_rate/100), 2)) as tax, CONCAT('£', FORMAT(SUM(gross) - SUM(materials), 2)) as nett, CONCAT('£', FORMAT(SUM(gross) - SUM((gross - materials)*deduction_rate/100), 2)) as payable FROM Payroll WHERE subcontractor_id = %d AND payment_date IS NOT NULL AND payment_date BETWEEN ",param.id);
...
dal.getKodeComAnnualInvoiceData=function(param,cb){
	...
	sql += "@invoiceCount :="
	sql += util.format("(SELECT COUNT(id) FROM Payroll WHERE subcontractor_id = %d AND payment_date IS NOT NULL and payment_date >= DATE_SUB(STR_TO_DATE('%s','%d-%m-%Y'),INTERVAL 1 YEAR))", param.id, param.yearEnd);
	sql += " as invoiceCount, "
